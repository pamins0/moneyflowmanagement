						...
						<div class="row">
							<form:form action="branchCashAvailabilityReport" id="NewEditForm"
								...
								<div class="form-group">
											...
											<%-- <div class="form-group">
												<label><fmt:message key="Common.form.BranchName"/></label>
												<input placeholder="Enter entity name" id="branchManagementName2"
													class="form-control" />												
											</div> --%>
							...
							aria-describedby="userListTable_info" role="grid">
								<thead>
									<tr class="info">
										...
										<th data-hide="phone"><fmt:message key="Report.list.Parameter" /></th>
										<th><fmt:message key="Report.list.Total" /></th>
										<th><fmt:message key="Report.list.ShowDenominations" /></th>
										<th><fmt:message key="Report.list.Status" /></th>										
										<th data-hide="phone, tablet"><fmt:message key="Report.list.CreatedTime" /></th>
									</tr>
								</thead>								
								...
								<tbody>
									...
									<c:forEach items="${auditTrials}" var="branch" varStatus="counter"> 
										<tr>											
											...
											<td><small>${branch.branchManagement.branchCode}</small></td>
											<td><small>${branch.branchParameter.parameterName}</small></td>
											<td><small>${branch.total}</small>
											<td><small>
													<a href='' data-toggle="modal"data-target="#myModalDenomination"
											...
											<td><small>${branch.status}</small></td>
											<td><small>${branch.createdTime}</small></td>											
										</tr>
									</c:forEach>
								</tbody>								
							</table>
...
<script>
	...
	// added later
	var baseUrl2 = "${pageContext.request.contextPath}/getChildBranchManagementsByBranchId";
	
	$("#branchManagementName").change(function() {
console.log($("#branchManagementName").val());
		//$("#branchManagementName").val("");
		//$("#branchManagementId").val("");
		//$('#designationId').empty();
		//getDesignationsByHierarchyId();
		
		$('#branchManagementName2').autocomplete({
			autoSelectFirst : true,
			serviceUrl : baseUrl2 + addBranchId(),
			paramName : "tagName",
			delimiter : ",",
			onSelect : function(suggestion) {
				$("#branchManagementId").val(suggestion.data);				
	...
	function addBranchId() {
		return '?branchId=' + $('#branchManagementId').val();
	}
