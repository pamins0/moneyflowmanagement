...
<sec:authorize access="hasRole('can_departmentmanagement_create')">
	<section class="content">
		<div class="row">
			<form:form id="NewEditDepartmentForm" method="POST"
				modelAttribute="department" class="form-inline">
				<form:hidden path="id" />
				<form:hidden path="requestFrom" />
				<c:choose>
					<c:when test="${empty department.urlReferer }">
						<c:set var="referer" value="${header['Referer']}" />
					</c:when>
					<c:otherwise>
						<c:set var="referer" value="${department.urlReferer}" />
					</c:otherwise>
				</c:choose>
					...
					<div class="box-header custom_header_form_page">
						<div class="heading-title-wrapper">
							...
							<c:choose>
								<c:when test="${edit}">
									<h3 class="box-title">
										Edit Department - <span class="edit-module-name">${department.title}
										</span>
									</h3>
								</c:when>
								<c:otherwise>
						...
						<c:set var="requestFrom" value="${department.requestFrom}" />
						<div class="btn-group custom_group" role="group"
							aria-label="Basic example">
							<c:choose>
								<c:when test="${department.requestFrom == 'byhierarchy'}">
							...
							<c:otherwise>
								...
								<sec:authorize access="!hasRole('can_view_everything')">
									...
									<c:if test="${edit==false}">
										<c:set property="hierarchyControl" target="${department}"
											value=""></c:set>
									</c:if>
						...
						<div class="form-group col-sm-3 w10">
							...
							<div class="col-sm-12">
								<div class="input-group">
									...
									<span class="has-error ">${duplicateDepartmentForHiearchy}</span>
								</div>
							</div>
						</div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		$("#NewEditDepartmentForm").submit(function() {
