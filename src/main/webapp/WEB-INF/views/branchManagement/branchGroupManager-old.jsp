		...
		<div class="container right_content_area">
			<div class="col-md-11 right_content_inner">
				...
				<form:form name="userRoleForm" id="userRoleForm"
					commandName="branchGroup" method="POST" class="aswd_dms_tbl"
					action="saveBranchGroupAssociation">
					...
					<div id="header-2" class="fixed-header-2">
						<div class="form-Assoc">
							...
							<div class="products col-sm-5 adding-section">
								<p class="heading">
									...
									<form:select id="insertedVA" path="insertedVA"
										...
										<form:options items="${deselectedBranchList}"
											itemLabel="branchName" itemValue="id" />
									</form:select>
								</p>
							</div>
								...
								<p class="heading">
									Selected Branches:<br> <select multiple="" size="8"
										...
										<c:forEach items="${selectedBranchList}" var="selectedBranch">
											<option value="${selectedBranch.branchManagement.id}">${selectedBranch.branchManagement.branchName}</option>
										</c:forEach>
									</select>
								</p>
							</div>
							...
							<p>
								...
								<a href='./closegroupmanagement' class="btn btn-success btn-lg"
									onclick=""> Cancel </a>
							</p>
							...
							<%int i = 0;%>
								...
								selectedArray[
							<%=i%>
								] = '${selectedBranch.branchManagement.id}';
							<%i++;%>
								</c:forEach>
								...
								function removeItem() {
									...
									for (j = toList.options.length - 1; j > -1; j--) {
										console.log("toList value at index: "
								...
								function saveData() {
									var toList = document
											.getElementById("selectedVA");
									//var selfBranchId = $("#selfBranchId").val();
									/* for (q = 0; q < toList.length; q++) {
										var optionVar = toList.options[q];
										var displayVar = $(optionVar).is(
												':hidden');
										//var display=$(optionVar).css('display')
										//alert(displayVar+"::"+display);
										if (!displayVar
												&& optionVar.value == selfBranchId) {
											selfBranch = true;
											break;
										}
									} */
