...
<sec:authorize access="hasRole('can_designationmanagement_create')">
	<%-- <c:if --%>
	<%-- 	test="${ct:isAllowed('can_designationmanagement_create',requestScope['scopedTarget.requestScopedPermissions'].permissions)}"> --%>
	<section class="content">
		<div class="row">
			<form:form id="NewEditDesignationForm" method="POST"
				modelAttribute="designation" class="form-inline">
				<form:hidden path="id" />
				<form:hidden path="requestFrom" />
				<c:choose>
					<c:when test="${empty designation.urlReferer }">
						<c:set var="referer" value="${header['Referer']}" />
					</c:when>
					<c:otherwise>
						<c:set var="referer" value="${designation.urlReferer}" />
					</c:otherwise>
				</c:choose>
				...
				<div class="col-md-12 table-container-main">
					<div class="box-header custom_header_form_page">
						<div class="heading-title-wrapper">
							...
							<c:choose>
								<c:when test="${edit}">
									<h3 class="box-title">
										Edit Designation - <span class="edit-module-name">${designation.title}
										</span>
									</h3>
								</c:when>
								<c:otherwise>
						...
						<c:set var="requestFrom" value="${designation.requestFrom}" />
						<div class="btn-group custom_group" role="group"
							aria-label="Basic example">
							<c:choose>
								<c:when test="${designation.requestFrom == 'byhierarchy'}">
					...
					<div class="box-body custom_form_body">
							...
							<c:otherwise>
								...
								<sec:authorize access="!hasRole('can_view_everything')">
									...
									<c:if test="${edit==false}">
										<c:set property="hierarchyControl" target="${designation}"
											value=""></c:set>
									</c:if>
						...
						<div class="form-group col-sm-3 w10">
							<label class="control-label col-sm-12 "><fmt:message
							...
							<div class="col-sm-12">
								<div class="input-group">
									...
									<span class="has-error ">${duplicateDesignationForHiearchy}</span>
								</div>
							</div>
						</div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		$("#NewEditDesignationForm").submit(function() {
...
<style>
.box-body.custom_form_body .button-group-set {
