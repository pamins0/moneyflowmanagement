...
<section class="content-header">
	<h1>Correspondent Branch Management</h1>
	<ol class="breadcrumb">
		<li><a><i class="fa fa-dashboard"></i> Home</a></li>
		<li class="active">Correspondent Branch</li>
	</ol>
</section>

<section class="content">
	<form:form name="userRoleForm" id="userRoleForm"
		onsubmit="return validateForm()" commandName="branchCorrespondentGroup"
		method="POST" class="aswd_dms_tbl" action="saveCorrespondentBranchAssociation">
		<div class="row">
				...
				<div class="box-header">
					...
					<div class="btn-group custom_group" role="group"
						...
						<p>
							...
							<a href='./branchmanagement' class="btn btn-danger btn-lg"
								onclick=""> Cancel </a>
						</p>
					</div>
						...
						<div class="box-table">
							<div class="box-header col-sm-4 ">
								<i class="halflings-icon white align-justify"></i><span
									class="break"></span>Associate Correspondent Branches For - ${branchCorrespondentGroup.parentCorrespondentBranch.branchName} <span
									class="edit-module-name"></span>								
								<form:hidden path="parentCorrespondentBranch.id" value="${parentCorrespondentBranch.id}" />
							</div>
									...
									Selected Branches:<br> <select multiple="multiple"
										...
										<c:forEach items="${selectedBranchList}" var="selectedBranch">
											<option value="${selectedBranch.id}"
												selected="selected">${selectedBranch.branchName}</option>
										</c:forEach>
									</select>
								</p>
							</div>							
							...
							<tbody>
								<c:forEach items="${selectedBranchList}" var="branch"
									...
									<tr>
										<td class="text-center"><input type="checkbox"
											checked="checked" value="${branch.id}"
											name="selectedVA" class="checkbox-branch"> <%-- ${counter.index+1} --%></td>
										<td><small>${branch.branchName}</small></td>
										<td><small>${branch.hierarchyControl.orgManagement.name}</small></td>
									</tr>
								</c:forEach>
								...
								<c:forEach items="${deselectedBranchList}" var="branch"
									...
									<tr>
										...
										<td><small>${branch.branchName} - ${branch.hierarchyControl.orgManagement.name}</small></td>
										<td><small>${branch.hierarchyControl.orgManagement.name}</small></td>
									</tr>
								</c:forEach>
							</tbody>
